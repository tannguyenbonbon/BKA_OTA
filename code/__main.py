...
class Tracker:
    def __init__(self):
        ...
        self.__hist_loc_rp_failed   = 0

        self.__is_parking               = False
        self.__last_reported_parking    = False
        self.__parking_speed_threshold  = (5.0)

        self.__apn_config           = ApnConfig()

        self.__usr_cfg              = settings.read("user")
    ...
    def __business_running(self):
        report_handlers = {
            "loc_report"                : self.__loc_report,
            "server_connect"            : self.__server_connect,
            "batt_report"               : self.__battery_info,
            "working_mode_report"       : self.__working_mode_report,
            "device_info_report"        : self.__device_info_report,
            "sim_info_report"           : self.__sim_info_report,
            "storage_info_report"       : self.__storage_info_report,
            "power_reset_reason_report" : self.__power_reset_reason,
        }
        ...
        while self.__business_tid is not None or self.__business_queue.size() > 0:
            ...
            if data[0] == 0:
                handler = report_handlers.get(data[1])
                if handler:
                    handler()
                else:
                    log.error("Unknown report type: {}".format(data[1]))
            elif data[0] == 1:
    ...
    def __loc_report(self):
        ...
        if server.status:
            ...
            self.__hist_loc_rp_failed += 1
            log.error("Location Report Failed: {}".format(self.__hist_loc_rp_failed))
        
        #History push failed to server > 5 mins -> reset
        #GPS not fix in 30 mins -> reset
        if(self.__hist_loc_rp_failed >= 10) or (self.__loc_state_failed >= 60):
    ...
    def __get_loc_data(self):
        def set_location_from_gps(res):
            return {
                "latitude"  : float(res["lat"]) * (1 if res["lat_dir"] == "N" else -1),
                "longitude" : float(res["lng"]) * (1 if res["lng_dir"] == "E" else -1),
        ...
        if self.__usr_cfg["loc_method"] & UserConfig._loc_method.gps:
            res = self.__gnss.read()
            satellites_data = {"satellites": res["satellites"], "state": res["state"]}
            if res["state"] == "A" and res["fix_mode"] != 1:
                loc_data.update(set_location_from_gps(res))
                loc_state = 1
    ...
    def __device_info_report(self):
        device_info = {
            "app_current_version" : self.__usr_cfg["ota_status"]["app_current_version"],
            "sys_current_version" : self.__usr_cfg["ota_status"]["sys_current_version"],
        }
    ...
    def __storage_info_report(self):
        sys_inf = SystemMonitor()
        storage_inf = sys_inf.get_storage_info
        if server.status:
            res = server.send_telemetry(storage_inf[0])
            res = server.send_telemetry(storage_inf[1])
            if not res:
                log.error("Failed to push storage infomation to server")
    ...
    def __server_connect(self):
        if not server.status:
            ...
            if not server.status:
                ...
                #Push start up report
                self.__push_startup_report()
        else:
            self.__server_reconn_count = 0
    ...
    def storage_info_report(self, args=None):
        self.__business_queue.put((0, "storage_info_report"))
