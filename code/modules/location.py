...
class NMEAParse:
    ...
    def FixMode(self):
        _gsa = self.GxGSAData
        fix_mode = _gsa[2] if _gsa else ""
        return fix_mode

    @property
    def Speed(self):
        _vtg = self.GxVTGData
        if _vtg and len(_vtg) > 7:
            try:
                speed = float(_vtg[7])  # Convert speed to float
            except ValueError:
                speed = 0.0  # Return 0 if conversion fails
        else:
            speed = 0.0  # Default to 0 if data is missing
        return speed
...
class GNSSInternal(GNSSBase):
    ...
    def __init__(self):
        ...
        if quecgnss.init() != 0:
            ...
            log.info("Quecgnss init success")
        # self._close()
    ...
    def _receive(self):
        ...
        log.debug("__GNSSInternal read start")

        while self.__running:
            gnss_data = quecgnss.read(1024)
            self._parse_loc(gnss_data[1] if (isinstance(gnss_data, tuple) and len(gnss_data) > 1 and gnss_data[1]) else b"")
            utime.sleep(1)
        ret = self._close()
        self.__tid = None
        self.__running_end = 0
        log.debug("__GNSSInternal read stop. %d" % ret)
